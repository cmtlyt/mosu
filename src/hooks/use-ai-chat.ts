...
8. DOM 变更通过 "domPatch" 字段表达，值为指令数组。未提供时保持当前 DOM 不变。
9. 每条指令包含 "op"（操作类型）、"selector"（CSS 选择器）及操作所需参数。
10. 支持的操作类型：
...
11. "html" 内容必须是纯 HTML 片段（不含 <html>/<body>/<head>），禁止 <script> 标签和事件属性（onclick 等）。
12. "selector" 必须基于当前预览区 DOM 摘要中存在的 class 或 id，确保可定位。
13. 首次创建场景时，使用 "add" 操作向 DOM 摘要中的第一个顶层元素（即根容器）添加完整结构，selector 使用该元素的 CSS 选择器（如 ".container" 或 "#root"）。
14. 完全重建 DOM 时，用 "replace" 直接替换根容器（selector 为根容器的 CSS 选择器），html 内容为全新的完整结构。这是最简洁可靠的方式。
15. 增量修改时仅输出变更部分的 patch 指令，不要重复未变更的内容。

## Style 规则

15. 当需要为预览区元素定义静态样式时，可输出 "style" 字段，值为合法的 CSS 字符串。
16. "style" 中严禁包含任何动画相关属性，包括但不限于：animation、animation-name、animation-duration、animation-timing-function、animation-delay、animation-iteration-count、animation-direction、animation-fill-mode、animation-play-state、transition、@keyframes。这些属性由 config.tracks 统一管理。
17. "style" 仅用于布局、颜色、字体、尺寸等静态视觉样式。`;
    ...
    try {
      ...
      if (parsed && typeof parsed === 'object') {
        if ('tracks' in parsed && !('config' in parsed)) {
          return { config: parsed as Pick<AnimationConfig, 'tracks'> & { name?: string } };
        }
        return parsed as AIEditorResponse;
      }
    } catch {
